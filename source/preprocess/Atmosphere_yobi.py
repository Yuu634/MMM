...
def get_Atmosphere(source_path):
    ...
    for filename in os.listdir(directory):
        ...
        if filename.endswith('.wav'):
            ...
            music_all = np.concatenate((music_all, music))
    ...
    """スペクトログラム取得"""
    D = librosa.stft(music_all,n_fft=n_fft,hop_length=hop_length,win_length=None)
    ...
    # index内容
    index = [(len(music_all)/fs)/len(count_list)*x for x in range(len(count_list))] # 秒数
